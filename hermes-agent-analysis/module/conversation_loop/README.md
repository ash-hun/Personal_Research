...
> 실제 코드는 ~4751줄짜리 거대 함수라 그대로 읽기 어렵습니다. 여기서는 **원본의 제어 흐름을
> 임의로 잘라내지 않고 충실히 복원**했습니다 — API 재시도 루프, 잘림(length) continuation,
> JSON 검증 재시도, guardrail halt, 빈응답 복구 사다리까지 모두 반영합니다.
>
> provider 폴백 체인·컨텍스트 압축·세션 영속화·plugin/memory/skill 같은 **외부 서브시스템**은
> (수천 줄의 결합 코드를 끌어오지 않기 위해) 삭제하지 않고 `AIAgent`의 **교체 가능한 hook**(기본
> no-op)으로 보존하고, 코드에 `[EXTERNAL SUBSYSTEM]` 배너로 원본 위치를 명시했습니다.
> 복원 항목·영역표시의 전체 매핑은 [LOOP_WALKTHROUGH.md](LOOP_WALKTHROUGH.md)의 "복원 기록"을 보세요.

...
```bash
# 관심 기능 폴더로 가서 데모 실행 (외부 API 키 불필요)
cd conversation_loop && python3 demo.py

# 12개 전체 데모를 한 번에 검증
for d in */ ; do (cd "$d" && python3 demo.py >/dev/null 2>&1 && echo "OK  $d" || echo "FAIL $d"); done
```

각 폴더의 `README.md`가 **구현방식 → 핵심 소스 매핑 → I/O 인터페이스 → 데이터·제어 흐름 → 커스터마이징 포인트** 순서로 정리되어 있습니다. 새 기능을 응용할 때는 해당 폴더의 "커스터마이징·응용 포인트" 섹션부터 보세요.
...
- 정리 범위는 **코어 엔진 + 시스템 기능** 중심입니다. UI(`ui-tui/`, `web/`), 스킬 콘텐츠 라이브러리(`skills/`의 개별 스킬들), 웹사이트는 "구현 기능"이 아니라 콘텐츠/프론트엔드라 제외했습니다.
- 각 미러는 **메커니즘의 충실한 distillation**이지 1:1 포팅이 아닙니다. 프로덕션 한정 관심사(멀티모달 처리, 시크릿 redaction, 파일 락, 폴백 체인 등)는 각 폴더 docstring에 명시하고 본체에서는 덜어냈습니다.
- LLM/외부서비스 호출 지점은 모두 **교체 가능한 callable(Protocol)** 로 빼서 mock 가능하게 했습니다 — 실제 응용 시 이 자리에 진짜 구현을 끼우면 됩니다.
